    ...
    caches.keys()
      .then(
        ...
        (keys) => Promise.all(
          keys.map(
            (key) => {
              // Удаляем только те кэши, которые начинаются с нашего префикса, но не совпадают по версии
...
const handleFetch = (evt) => {
  ...
  evt.respondWith(
    caches.match(request)
      .then((cacheResponse) => {
        // Если в кэше нашёлся ответ на запрос (request), возвращаем его (cacheResponse) вместо запроса к серверу
        if (cacheResponse) {
          return cacheResponse;
        }

        // Если в кэше не нашёлся ответ, повторно вызываем fetch с тем же запросом (request), и возвращаем его
        return fetch(request)
          .then((response) => {
            // Если ответа нет, или ответ со статусом отличным от 200 OK, или ответ небезопасного типа (не basic), тогда просто передаём
